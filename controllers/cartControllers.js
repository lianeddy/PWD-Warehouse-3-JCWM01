...
module.exports = {
  getCartUser: async (req, res, next) => {
    ...
    const querySelectCart = `SELECT URL, nm_master_produk AS ITEM, quantity AS QTY, harga AS PRICE, app_carts_produk.quantity * app_master_produk.harga AS TOTAL FROM app_carts_produk INNER JOIN app_master_produk ON app_carts_produk.id_master_barang = app_master_produk.id_master_produk INNER JOIN sys_user ON app_carts_produk.id_user = sys_user.id_user WHERE app_carts_produk.id_user = ?`;
